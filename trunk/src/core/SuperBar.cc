...
/* Super Constructor */ /*{{{*/
SuperBar::SuperBar(XWin *win, string barImg, string barFont, int iSize,
  ...
  if (filtSel != 0) {
    ...
    */
    ...
    imlib_filter_divisors(255, filtRed != 0 ? 3 * 255 / filtRed : 1,
                          filtGreen != 0 ? 3 * 255 / filtGreen : 1,
                          filtBlue != 0 ? 3 * 255 / filtBlue : 1);
#endif
...
void SuperBar::updateLength(int newSize) { icons.resize(newSize); }
...
void SuperBar::cleanBack() {
  if (font && drawfont)
    if (rest_w > 0) {
      USE_IMAGE(buffer);
      BLEND_IMAGE(font_restore, 0, 0, rest_w, rest_h, rest_x, rest_y, rest_w,
                  rest_h);
      rest_w = 0;
    }
    ...
    if (cur_ic->need_update == 1) {
      ...
      if (font && drawfont) {
        ...
        if (a == icons.size() - 1 && zoomed_icon != -1) {
          ...
          BLEND_IMAGE(buffer, rest_x, rest_y, rest_w, rest_h, 0, 0, rest_w,
                      rest_h);
