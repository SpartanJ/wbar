...
Bar::Bar(XWin *win, string barImg, int iSize, int iDist, float zFactor,
    ...
    : /* Initialization */
      ...
      icon_size(iSize), icon_anim(nAnim), jump_factor(jFactor),
      zoom_factor(zFactor), orientation(bOrient), position(bPosition),
      zoomed_icon(-1), focused(1) {
    ...
    if (!(bar = LOAD_IMAGE(barImg.c_str()))) {
        throw(barImg + _(" Image not found. Maybe using a relative path?"))
            .c_str();
    }
...
void Bar::scale(bool updateBG) {
    ...
    icon_offset =
        icon_ansd * ((zoom_factor > 1) ? (zoom_factor - 1) : 0.5) * icon_unit;
    /* Icon scaling constant */
    b_dd = (zoom_factor > 1) ? (zoom_factor - 1) * icon_unit
                             : zoom_factor * icon_unit;
...
#else
    /* Estos calculos de escala son en base a la funcion sigmoidea f =
     * 1/(1+exp(a+bx^2))*/
    b_scl_c = 0.01; // mientras mas cercano a 0 mas se desplaza chico
    b_scl_d = 0.20; // mientras mas cercano a 1 mas se desplaza grande -> si
                    // quiero pico lo achico
...
void Bar::transform(int mousex) {
    ...
    for (a = 0; a < (int) icons.size(); a++, rx -= icon_unit) {
        ...
        if (((a < i_num) ? i_num - a : a - i_num) > icon_ansd) {
            ...
            if (cur_ic->size != icon_size || cur_ic->x != t_x ||
                cur_ic->y != t_y) {
...
#ifdef COS_ZOOM
            cur_ic->size = (int)(
                icon_size * (1.0 + (zoom_factor - 1.0) * cosf(b_scl_b * rx)));
#else
            cur_ic->size =
                (int)(icon_size *
                      (1.0 + (zoom_factor - 1.0) /
                                 (1.0 + expf(b_scl_a + b_scl_b * rx * rx)) /
                                 b_scl_d));
#endif

#ifdef LINEAR_TRASL
            cur_ic->x =
                cur_ic->ox - (int)(icon_ansd * b_dd - b_pos_m * (mousex - x0));
#else
            cur_ic->x =
                cur_ic->ox + (int)(icon_ansd * b_dd * sinf(b_pos_m * rx));
#endif
...
void Bar::cleanBack() {
    ...
    for (size_t a = 0; a < icons.size(); a++) {
        ...
        if (orientation == 0)
            BLEND_IMAGE(cleaning_buffer, cur_ic->cx, 0, cur_ic->csize,
                        window->h, cur_ic->cx, 0, cur_ic->csize, window->h);
        else
            BLEND_IMAGE(cleaning_buffer, 0, cur_ic->cx, window->h,
                        cur_ic->csize, 0, cur_ic->cx, window->h, cur_ic->csize);
    }
}
...
void Bar::drawBack() {
    ...
    if (orientation == 0) {
        BLEND_IMAGE(barback, 0, 0, window->w, window->h, 0, 0, window->w,
                    window->h);
...
void Bar::expand(bool inverse) {
    ...
    while (t < anim_time) {
        ...
        t = (int)((tv.tv_sec - tv0.tv_sec) * 1000 +
...
inline void Bar::iconPress(int i_num, int offs) {
    ...
    if (i_num < 0 || i_num >= (int) icons.size()) {
        return;
    }
...
void Bar::iconUp(int i_num) { iconPress(i_num, -2); }

void Bar::iconDown(int i_num) { iconPress(i_num, 2); }
/*}}}*/

/* Position the Bar */
void Bar::setPosition(string pos) {

    if (!pos.compare("bottom")) {
        position = 1;
    } else if (!pos.compare("top")) {
        position = 2;
    } else if (!pos.compare("left")) {
        position = 3;
    } else if (!pos.compare("right")) {
        position = 4;
    } else if (!pos.compare("center")) {
        position = 5;
    } else if (!pos.compare("bot-right")) {
        position = 6;
    } else if (!pos.compare("bot-left")) {
        position = 7;
    } else if (!pos.compare("top-right")) {
        position = 8;
    } else if (!pos.compare("top-left")) {
        position = 9;
    } else {
...
int Bar::iconsShown() { return (int) icons.size(); }
...
float Bar::getZoom() { return b_zoom_f; }
/*}}}*/
#endif
...
static int rootErrorHandler(Display * display, XErrorEvent * theEvent) {
    if (theEvent->error_code == 9 && theEvent->request_code == 14) {
        ...
        std::cout << "you may work around that by setting a wallpaper"
                  << std::endl;
    }
